...
function runInTerminal(command, name = "Monorepo Assistant") {
...
function registerSchematicsCommands(context) {
  const genDomain = vscode2.commands.registerCommand(
    "angular-monorepo-assistant.generateDomain",
    async (uri) => {
      const defaultDomain = getDomainFromUri(uri);
      const appName = await vscode2.window.showInputBox({
        prompt: "Enter the App name",
        value: "app"
      });
      if (appName === void 0) return;
      const domainName = await vscode2.window.showInputBox({
        prompt: "Enter the Domain name",
        value: defaultDomain || "",
        placeHolder: "e.g., products, orders"
      });
      if (!domainName) return;
      const featureName = await vscode2.window.showInputBox({
        prompt: "Enter the initial Feature name",
        value: "list",
        placeHolder: "e.g., list, details"
      });
      if (!featureName) return;
      const routingOpt = await vscode2.window.showQuickPick(["Yes", "No"], {
        placeHolder: "Add lazy-loaded route?"
      });
      if (routingOpt === void 0) return;
      const navigationOpt = await vscode2.window.showQuickPick(["Yes", "No"], {
        placeHolder: "Add to navigation menu?"
      });
      if (navigationOpt === void 0) return;
  ...
  const genFeature = vscode2.commands.registerCommand(
...
function registerTaskCommands(context) {
  const startCmd = vscode3.commands.registerCommand("angular-monorepo-assistant.runStart", () => {
    runInTerminal("pnpm start", "Monorepo Dev Server");
  });
  const testCmd = vscode3.commands.registerCommand("angular-monorepo-assistant.runTest", () => {
    runInTerminal("pnpm test", "Monorepo Tests");
  });
  const lintCmd = vscode3.commands.registerCommand("angular-monorepo-assistant.runLint", () => {
    runInTerminal("pnpm lint", "Monorepo Linting");
  });
  const formatCmd = vscode3.commands.registerCommand("angular-monorepo-assistant.runFormat", () => {
    runInTerminal("pnpm format", "Monorepo Prettier");
  });
  context.subscriptions.push(startCmd, testCmd, lintCmd, formatCmd);
}

// src/views/dashboardWebview.ts
var vscode4 = __toESM(require("vscode"));
var fs2 = __toESM(require("fs"));
var path = __toESM(require("path"));

// src/utils/sheriffParser.ts
var fs = __toESM(require("fs"));
function parseSheriffConfig(filePath) {
  ...
  while ((match = entryRegex.exec(depRulesBlock)) !== null) {
    ...
    const deps = rawDeps.split(",").map((d) => d.trim()).filter((d) => d.length > 0).map((d) => {
...
function writeSheriffConfig(filePath, rules, originalContent) {
  ...
  const lines = Object.entries(rules).map(([tag, deps]) => {
    const formattedDeps = deps.map((dep) => dep === "sameTag" ? "sameTag" : `"${dep}"`).join(", ");
...
${lines.join(",\n")}
  ...
  const updatedContent = originalContent.substring(0, depRulesStartIndex) + newBlock + originalContent.substring(blockEndIndex + 1);
  fs.writeFileSync(filePath, updatedContent, "utf-8");
}

// src/views/dashboardWebview.ts
var DashboardWebviewPanel = class _DashboardWebviewPanel {
  static currentPanel;
  _panel;
  _extensionUri;
  _disposables = [];
  static createOrShow(extensionUri) {
    ...
    if (_DashboardWebviewPanel.currentPanel) {
      _DashboardWebviewPanel.currentPanel._panel.reveal(column);
      _DashboardWebviewPanel.currentPanel._updateData();
      return;
    }
    ...
    _DashboardWebviewPanel.currentPanel = new _DashboardWebviewPanel(panel, extensionUri);
  }
  ...
  dispose() {
    _DashboardWebviewPanel.currentPanel = void 0;
    this._panel.dispose();
  ...
  _updateData() {
    ...
    let sheriffRules = {};
    let sheriffError = null;
    const sheriffPath = path.join(root, "sheriff.config.ts");
    try {
      if (fs2.existsSync(sheriffPath)) {
    ...
    try {
      if (fs2.existsSync(flagsPath)) {
        flagsConfig = JSON.parse(fs2.readFileSync(flagsPath, "utf-8"));
      } else {
  ...
  _handleRunTask(task, args) {
    if (task === "start") {
      vscode4.commands.executeCommand("angular-monorepo-assistant.runStart");
    } else if (task === "test") {
      vscode4.commands.executeCommand("angular-monorepo-assistant.runTest");
    } else if (task === "test:ui") {
      runInTerminal("pnpm test:ui", "Monorepo Tests UI");
    } else if (task === "lint") {
      vscode4.commands.executeCommand("angular-monorepo-assistant.runLint");
    } else if (task === "format") {
      vscode4.commands.executeCommand("angular-monorepo-assistant.runFormat");
  ...
  _handleSaveSheriff(rules) {
    const root = getWorkspaceRoot();
    if (!root) return;
    const sheriffPath = path.join(root, "sheriff.config.ts");
    try {
      const original = fs2.readFileSync(sheriffPath, "utf-8");
      writeSheriffConfig(sheriffPath, rules, original);
      vscode4.window.showInformationMessage("Sheriff boundaries updated successfully!");
      this._updateData();
    } catch (err) {
      vscode4.window.showErrorMessage(`Failed to save Sheriff boundaries: ${err.message}`);
    }
  }
  _handleSaveFlags(config) {
    const root = getWorkspaceRoot();
    if (!root) return;
    const flagsPath = path.join(root, "apps", "app", "public", "flagsConfig.json");
    try {
      fs2.writeFileSync(flagsPath, JSON.stringify(config, null, 2), "utf-8");
      vscode4.window.showInformationMessage("Feature Flags configuration updated successfully!");
      this._updateData();
    } catch (err) {
      vscode4.window.showErrorMessage(`Failed to save Feature Flags: ${err.message}`);
    }
  }
  _getHtmlForWebview() {
    return `<!DOCTYPE html>
  ...
  <div class="container">
    ...
    <div id="dashboard-tab" class="tab-content active">
      ...
      <div class="grid">
        ...
        <div class="card">
          <h3 class="card-title">\u{1F680} Development Tasks</h3>
        ...
        <div class="card">
          <h3 class="card-title">\u{1F4E6} Visual Schematic Generator</h3>
...
var SidebarProvider = class {
  ...
  static viewType = "angular-monorepo-sidebar";
  _view;
  resolveWebviewView(webviewView, _context, _token) {
    ...
    webviewView.webview.onDidReceiveMessage((data) => {
      switch (data.type) {
        case "openDashboard":
          vscode5.commands.executeCommand("angular-monorepo-assistant.openDashboard");
          break;
        case "runTask":
          if (data.task === "start") {
            vscode5.commands.executeCommand("angular-monorepo-assistant.runStart");
          } else if (data.task === "test") {
            vscode5.commands.executeCommand("angular-monorepo-assistant.runTest");
          } else if (data.task === "lint") {
            vscode5.commands.executeCommand("angular-monorepo-assistant.runLint");
          } else if (data.task === "format") {
            vscode5.commands.executeCommand("angular-monorepo-assistant.runFormat");
          }
          break;
      }
    });
...
<body>
  <button class="main-btn" onclick="openDashboard()">\u{1F5A5}\uFE0F Open Full Dashboard</button>
...
function activate(context) {
  ...
  const dashboardCmd = vscode6.commands.registerCommand(
  ...
  const sidebarReg = vscode6.window.registerWebviewViewProvider(
